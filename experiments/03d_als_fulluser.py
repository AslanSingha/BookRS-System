"""
Experiment 3d: ALS on full users (446K) to complete the comparison
"""
import pandas as pd
import numpy as np
...
print(f"Books: 2,000 | Users: {len(active_users):,} | Interactions: {len(interactions):,}")

user_ids = interactions["user_id"].unique()
book_ids = interactions["book_id"].unique()
user2idx = {u: i for i, u in enumerate(user_ids)}
book2idx = {b: i for i, b in enumerate(book_ids)}
n_users = len(user_ids); n_books = len(book_ids)
...
print(f"\nALS (full users): P@10={p} | R@10={r} | MRR={mrr}")

# Combine with existing full-user results
existing = pd.read_csv(OUTPUT_DIR / "cf_comparison_fulluser.csv")
...
final.to_csv(OUTPUT_DIR / "cf_comparison_fulluser.csv", index=False)

print("\n=== FINAL CF COMPARISON (Full Users) ===")
